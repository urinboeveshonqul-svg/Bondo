...
**Phase:** 3B Supabase integration — 🟡 **partially complete.** K-3 closed, the
service layer exists; wiring is blocked on **K-15** and a live project.
**Overall progress:** ~55%
...
| Translations                           | **14 namespaces × 3 locales, 692 keys each**                                                                       |
| First Load JS — home                   | **143 kB**                                                                                                         |
| First Load JS — listing / detail       | 120 kB / 131 kB                                                                                                    |
| First Load JS — admin dashboard        | 134 kB                                                                                                             |
| First Load JS — admin, heaviest        | 193 kB (brands: table + dialog editor)                                                                             |
| Shared JS                              | 103 kB                                                                                                             |
| Middleware bundle                      | **105 kB**                                                                                                         |
| Static prerendered routes              | **78** — 26 routes × 3 locales                                                                                     |
...
| #    | Issue                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Severity                                  | Plan                                                                                                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| K-1  | `/admin` is protected by authentication only; any signed-in customer passes middleware, which does not check roles. **Thirteen admin routes now exist.** They are unreachable in production — `NODE_ENV` gates the preview (ADR-45) and `/admin` still redirects to a `/sign-in` page that does not exist — so nothing is exposed today. The danger is the gate being removed before the role check replaces it.                                                                                           | **High (latent)**                         | The auth phase, as its first task: a role check in the admin layout, backed by RLS, replacing `isAdminPreview` in `supabase/session.ts`                                          |
| K-2  | Middleware redirects to `/sign-in`, which does not exist, so protected routes currently 404 after redirect.                                                                                                                                                                                                                                                                                                                                                                                                | Low                                       | Phase 2                                                                                                                                                                          |
| K-16 | **The admin's vocabulary does not match the schema's enums.** The admin models publishing as `draft \| published \| hidden`; the database has `product_status` (`draft \| active \| archived`) **and** a separate `product_visibility` (`public \| hidden`). Inventory reasons differ too — the UI offers `damage` and `recount`, the enum has `correction`. The interface therefore offers states the database cannot store. Found by the generated types, not by reading.                                | **High — blocks wiring the admin**        | Map in the service layer or migrate the enums. It is the first task of wiring the admin.                                                                                         |
| K-15 | **The schema cannot store the application's content model.** `products.name`, `products.description`, `categories.name` and every other content column are single `text`. The app models all of them as `LocalizedText` (uz/ru/en) — **ADR-39**. There are no translation tables, so mapping a row onto the UI's types would mean inventing two languages.                                                                                                                                                 | **High — blocks replacing the mocks**     | A migration adding `*_translations` tables and a per-locale search vector, designed in ROADMAP § "Translated content in the database". This is the gate on the rest of Phase 3B. |
...
| D-18 | The service layer is written but **unexercised**. Every query is type-checked against the generated schema and none has ever run: no Supabase project exists, so no `select` has returned a row and no RLS policy has refused one. Type-correct is not the same as correct.                                                                                                                            | **High — a query can be wrong in ways types cannot see**                                           | Run against a real project as the first task once one exists; the assertions to make are listed in ROADMAP § Phase 3B                           |
| D-17 | The PGlite harness stubs `auth` and `storage`, so generated types and schema assertions describe the `public` schema only. A hosted project could drift from the migrations without either noticing.                                                                                                                                                                                                   | Medium — grows with every migration applied outside this repo                                      | `db:types:remote` once linked; a diff against the committed file is the drift signal                                                            |
...
**Close K-15 — give the schema a place to put translations.** Everything else in
Phase 3B is behind it: the services compile and cannot be wired, because a
`products.name` of type `text` has nowhere to hold three languages.

Write the migration designed in
[ROADMAP § Translated content in the database](ROADMAP.md#translated-content-in-the-database-adr-39):

1. `product_translations`, `category_translations`, `brand_translations`,
   `banner_translations` — `(<entity>_id, locale)` primary key, cascading
   delete, RLS mirroring the parent table's policies.
2. Decide the `search_vector` model. One `tsvector` with the `simple` config
   cannot serve three languages; the choice is a column per locale or a vector
   on the translation row. Record it as an ADR either way.
3. Move the existing single-language columns' data into the `en` row, then drop
   them — in that order, in one migration, so no deploy has both shapes.
4. `npm run db:types && npm run db:verify` — the generated types must show the
   new tables, and the assertions must still pass.

Then **K-16**: reconcile the admin's `draft | published | hidden` with
`product_status` + `product_visibility`, and its inventory reasons with
`inventory_movement_type`. Map in the service layer or migrate the enums;
decide before wiring, not during.

Wiring itself needs a Supabase project — `.env.local`, `supabase link`,
`db push`. Until one exists no query can be executed, only compiled (**D-18**).
