...
| D-15 | `mocks/admin.ts` still backs **inventory, homepage, content pages, users and settings**. Products, categories and brands no longer read it, and the command palette no longer does either. The remaining five are declared `persistence: "fixtures"` in the registry, so the panel names them on screen rather than implying the whole admin is connected.                                                                                                                                                                                                                                                    | Medium — down from High; the modules that handled money-adjacent data are now real                                                                                                   | Each remaining module, in the P1 order: orders, media, settings                                                                                  |
...
| Inventory      | ❌ fixtures  | ❌                                              | `inventoryRecords`, `inventoryMovements` |
| Homepage       | ❌ fixtures  | ❌                                              | `banners`, `homepageSections`            |
| Content pages  | ❌ fixtures  | ❌                                              | `contentPages`, `getContentPage`         |
| Users          | ❌ fixtures  | ❌                                              | `adminUsers`                             |
| Settings       | ❌ fixtures  | ❌                                              | `storeSettings`                          |

The command palette now reads products, categories and brands from the database
rather than from fixtures. Only the content-pages group there is still a mock.
