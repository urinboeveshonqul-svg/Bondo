...
🟢 **Deployed to a hosted project.** 🔴 **The catalog is empty — genuinely, as of
2026-08-14.** Every product, brand and order in the project was a test fixture
and all of them are now retired. Live products: 0. Live brands: 0. Live orders: 0. The dashboard reads zero everywhere because that is the true state of the
business, not because anything is hardcoded.
...
| Migrations applied  | 🟢 **all 26**, local and remote in lockstep                                    |
...
| #    | Issue                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Severity                                                                                                                                | Plan                                                                                                                                                                                                                                                                                                                           |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| K-27 | **`www.bondo.uz` fails TLS.** The hostname resolves but the certificate does not cover it, so a visitor typing `www.` gets a browser security warning instead of the shop. `bondo.uz` itself is correct and serves production.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Medium — it only affects visitors who type the `www.` prefix, but for those it looks like a compromised site.                           | Either add `www.bondo.uz` in Vercel → Domains as a redirect to the apex, so Vercel issues a covering certificate, or remove the DNS record. Half-configured is the worst of the three states.                                                                                                                                  |
| K-26 | **The verification harnesses write to the production database.** `scripts/verify-admin-crud.mjs` and `order-e2e-seed.mjs` read `.env.local` and run against the hosted project. This is the root cause of every fixture removed on 2026-08-14: 11 products, 12 brands, 5 orders, 11 permanent ledger rows and 5 active Super Administrator accounts, one of which reached the public storefront.                                                                                                                                                                                                                                                                                                                                                          | **High** — every run pollutes the live shop, and `inventory_movements` rows can never be removed.                                       | Point them at a separate Supabase project, or gate them behind an explicit environment check that refuses to run when the URL is the production ref.                                                                                                                                                                           |
| K-28 | ~~Five `CRUD tester` Super Administrator accounts were active on production.~~ **Closed** 2026-08-14. Created by `verify-admin-crud.mjs`, all `is_active = true` with the Super administrator role and full permissions. Deactivated with `is_active = false`; the `auth.users` rows were left in place.                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Resolved                                                                                                                                | —                                                                                                                                                                                                                                                                                                                              |
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| D-36 | **`mocks/catalog.ts` and its fallback are dead code with a misleading comment.** `withFixtureFallback` still wraps six reads in `services/catalog.reads.ts`, and its docstring says "There is no Supabase project yet (**D-18**)" and "Delete the fallback the day a project exists". A project has existed since 2026-08-04. The guard itself is sound — it rethrows when `NODE_ENV === "production"` and imports lazily, so fixtures cannot reach a deployed build.                                                                                                                                         | Low — it cannot ship fake data. But the comment actively misinforms the next reader about whether a database exists.                                                                 | Delete `mocks/catalog.ts`, unwrap the six call sites, and remove the helper.                                                                     |
| D-35 | **`cleanup-harness.mjs` only matches the `CRUD-` fixtures.** Its selectors are `sku like 'CRUD-%'`, `slug like 'crud-brand%'`, `key like 'crud-page-%'` and `notes like 'crud check%'`. The `E2E-` fixtures from `order-e2e-seed.mjs` match none of them, which is exactly why an E2E product stayed live on `bondo.uz` for two days after cleanup was believed to have run.                                                                                                                                                                                                                                  | Medium — it grows every time the E2E harness runs, and the failure is silent: the script reports success having cleaned nothing.                                                     | Widen the selectors, or make each harness own its teardown and run both. Resolved for good by fixing **K-26**.                                   |
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADR-88 | **A published product's stock level is readable by anyone, exact quantity included.** `20260819001000_public_stock_visibility.sql` adds one `select` policy on `inventory` for `anon` and `authenticated`, scoped by `is_product_published(product_id)`. `inventory_movements` stays staff-only.                                                                                                                                                                               | Without it the storefront rendered every product out of stock with its basket disabled, because `inventory` had only `to authenticated` policies gated on the `inventory.read` staff permission — nothing could be bought by anybody. Scoping by the same predicate the product policies use means a stock row is readable exactly when its product is, so no new surface opens. The cost is real and named rather than hidden: a competitor can read unit counts. Hiding the figure needs a view or a generated column and a second read path, and the storefront already displays a value derived from the number. Revisit with a view, not by widening this policy.                                                                                                                                                                                                                                                                                                                                                                                                             |
| ADR-87 | **An order is retired by setting `deleted_at`, never deleted.** `20260820001000_order_soft_delete.sql` adds the column; the ten order reads filter `deleted_at is null`; RLS is unchanged so `orders.read` still returns retired orders.                                                                                                                                                                                                                                       | Deletion is not merely discouraged here, it is impossible: `order_status_history` is append-only via `reject_ledger_mutation()`, which fires for every role including `service_role`, and its `order_id` cascades from `orders` — so any `delete from orders` aborts on the ledger trigger. That is correct, because the timeline is the evidence the order happened. Retiring an order is not a claim that it never did, so the timeline is kept. The filter sits in the services rather than in RLS because support must still be able to look a retired order up by reference, and per query rather than in `applyFilters` because that helper carries the operator's own filters and is skipped by eight of the ten reads — a visibility rule hidden there would silently leak retired rows.                                                                                                                                                                                                                                                                                   |
| ADR-86 | **A module's copy lives in its own namespace, and a storefront-facing table needs a storefront consumer.** Homepage copy moved from `adminContent.homepage` to `adminHomepage`; `components/home/promo-banners.tsx` renders active `home_hero` banners.                                                                                                                                                                                                                        | Two failures with one root. The admin screen printed `adminHomepage.title` because the page asked for a namespace that did not exist — next-intl's failure mode for a missing namespace is to render the key path, which looks like a missing translation and is actually a missing _file_. Splitting the namespace to match the registry entry makes the two impossible to drift again. The second: banners were fully manageable and completely invisible, because no page read them. A switch that changes a row and nothing else is indistinguishable from a switch that does nothing, and it is the kind of gap that survives every green test suite — the CRUD checks passed throughout.                                                                                                                                                                                                                                                                                                                                                                                     |
| ADR-85 | **The order module is screens over the existing schema, and the workflow is forward-only.** `/admin/orders` and `/admin/orders/[id]` render `orders.service`; transitions come from `nextOrderStatuses`, which offers the next step and `cancelled` and nothing else. Reviews are written from the account's order detail, not only from the product page.                                                                                                                     | Nothing about orders needed building except the interface: the schema, the RLS, `place_order`, the claim token and the services were all in place and verified. Registering the module before the routes existed would have put a sidebar link to a 404 in front of every operator holding `orders.read`, which is why it was held back. Forward-only is the load-bearing decision: `delivered` is what unlocks the customer's right to review (ADR-66), so an undo button would strand reviews that already exist — an operator who marks an order delivered by mistake needs a colleague and the audit trail, not a silent reversal. Cancelling is the one irreversible move and the one behind a confirmation.                                                                                                                                                                                                                                                                                                                                                                  |
...
| Homepage banners      | ✅ Editor over `site_banners`, and the storefront renders the active ones above the hero (**ADR-86**)                  |
| Orders (admin)        | ✅ `/admin/orders` and `/admin/orders/[id]` — queue, filters, full workflow, internal note, contact links (**ADR-85**) |
| Reviews (customer)    | ✅ Written from the account order detail once an order is `delivered`; gated by RLS, not by the form (**ADR-66**)      |
...
**Stop the harnesses writing to production (K-26), then put real products in.**

The orders migration is pushed and the screens exist. What is now blocking a
real shop is that the database has nothing in it and the tooling refills it with
fixtures.

1. **Fix K-26 first.** `scripts/verify-admin-crud.mjs` and `order-e2e-seed.mjs`
   read `.env.local` and write to the hosted project. Until that changes, every
   verification run puts fake products, fake orders and active Super
   Administrator accounts back into the live shop, and each one leaves a
   permanent `inventory_movements` row that cannot be deleted. Either point them
   at a second Supabase project or make them refuse to run against the
   production ref.
2. **Populate the catalog.** Zero live products, zero live brands. This is a
   product decision, not a technical one: real products entered through the
   admin, which persists. Not `seed.sql`, and not fixtures (**ADR-20**).
3. **Decide whether Bondo tracks stock.** `services/catalog.reads.ts` says "this
   shop does not track stock — a published product is orderable, full stop",
   while `inventory` exists, is read into `ProductSummary.stock`, and now has a
   public read policy exposing exact quantities (**ADR-88**). Both cannot be
   true. The answer removes either a table or a policy.
4. **Commit the in-flight checkout work.** `components/cart/buy-now.tsx` is
   untracked and exists only on one machine; `actions/orders.actions.ts`,
   `app/[locale]/checkout/page.tsx` and `components/checkout/checkout-form.tsx`
   carry uncommitted changes that `getLatestOrderDetails` was added for.
5. **Gitignore the harness artifacts.** `.order-e2e.json`, `probe-products.mjs`,
   `order-e2e-seed.mjs` and `cleanup-harness.mjs` are untracked and unignored —
   one `git add -A` from being committed. They are also the only reason
   `npm run check` currently fails, at `format:check`.
