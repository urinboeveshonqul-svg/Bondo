...
**Decide what goes into the empty catalog, then wire the admin to write it.**
The backend is operational: the project is linked, all 11 migrations are
applied, the schema matches the committed types exactly, and the storefront
reads it successfully in all three locales. It simply has no rows to show.

1. **Populate the catalog.** Two honest options, and it is a product decision
   rather than a technical one:
   - real products entered through the admin once it persists, or
   - `supabase/seed.sql` pushed deliberately as a staging fixture — it is
     development data (ADR-25) and does not belong in a store that will take
     orders.
2. **Close K-1 before anything writes.** The role check in the admin layout,
   backed by RLS. `admins` is empty, so the first administrator has to be
   inserted deliberately alongside it.
3. **Then the admin's Server Actions** — each form's `onSubmit` becomes a
   `createAction()` call in `actions/admin/<id>.actions.ts`, and
   `mocks/admin.ts` is deleted (**D-15**, **D-16**).

Still unexercised, and only rows or a session will exercise them (**D-18**):
the fold from `*_translations` onto `LocalizedText`, every write path, RLS
refusals for an authenticated non-admin, and storage object policies
(**K-8**).
