...
| ADR-32 | Env URLs are validated with `z.httpUrl()` / an explicit scheme check, not `z.url()` / `URL.canParse()`; and a value with leading or trailing whitespace is rejected rather than trimmed.                          | Both loose checks accept `postgresql://postgres:…@db.<ref>.supabase.co:5432/postgres`, the connection string Supabase shows beside the project URL. It would validate and then fail at the first query. Whitespace from a paste survives every truthiness and length check, making the credential quietly wrong at runtime; trimming it silently would hide the mistake from whoever can fix it at source. |
