...
**Phase:** 3D Admin module architecture ✅ **Complete.** Every admin module now
derives from one registry and one component kit; adding a module is a record
plus a folder.
**Overall progress:** ~66%
...
Phase 3B Storefront Data Wiring    ██████████████░░░░░░  70%   (services + catalog reads; needs a project)
...
| Migrations           | 11, in `supabase/migrations/`                                        |
...
**Create a Supabase project and run the service layer against it.** It is now
the only thing standing between the panel and real data. The architecture, the
schema, the types and the services are all in place and none of them has ever
executed a query (**D-18**).

1. `.env.local` from `.env.example`, then `supabase link` and `supabase db push`.
2. `npm run db:types:remote` and diff against the committed file. **Any diff is
   schema drift** and the migrations are right (**D-17**).
3. Work the assertion list in
   [ROADMAP § To verify once a project exists](ROADMAP.md#to-verify-once-a-project-exists-d-18):
   every service function against real rows, RLS refusing what it should for
   each of the five roles, storage uploads against the five buckets (**K-8**),
   and the seed applying against real GoTrue (**K-9**).

Then wire the admin: each module already renders `types/admin.ts` shapes, so
each screen is a service call replacing a `mocks/admin.ts` import, and each
form's `onSubmit` becomes a `createAction()` call in
`actions/admin/<id>.actions.ts` (**D-15**, **D-16**).

**Not before K-1.** The role check in the admin layout, backed by RLS, is the
first task of the auth phase and it precedes anything that persists. The module
guard added in Phase 3D is defence in depth, not the boundary.
